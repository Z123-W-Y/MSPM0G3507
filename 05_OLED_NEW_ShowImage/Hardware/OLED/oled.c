...
void OLED_ShowChinese(uint8_t x,uint8_t y,uint8_t no,uint8_t sizey)
{
    ...
    for(i=0;i<size1;i++)
    {
        ...
        // if(sizey==16) OLED_WR_Byte(zwy[no][i],OLED_DATA);//16x16字号
        // else if(sizey==8) OLED_WR_Byte(xxx[c][i],OLED_DATA);//用户添加字号
        else return;
    }				
}
...
{ 	
    ...
    for(i=0;i<Height;i++)
    {
        ...
        for(m=0;m<Width;m++)
        {      
            OLED_WR_Byte(Image[j++],OLED_DATA);	  
        }
    }
} 
...
void OLED_MyChinese(uint8_t x,uint8_t y,char* Chinese)
{
	...
	for (i = 0; Chinese[i] != '\0'; i ++)		//遍历汉字串
	{
		...
		if (pChinese >= OLED_CHN_CHAR_WIDTH)
		{
			...
			for (pIndex = 0; strcmp(OLED_16x16[pIndex].Index, "") != 0; pIndex ++)
			{
				/*找到匹配的汉字*/
				if (strcmp(OLED_16x16[pIndex].Index, SingleChinese) == 0)
				{
			...
			OLED_DrawImage(x + ((i + 1) / OLED_CHN_CHAR_WIDTH - 1) * 16, y, 16, 16, OLED_16x16[pIndex].Data);
		}
	}
}
